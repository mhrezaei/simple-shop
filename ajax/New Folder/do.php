...
//safety...
//if(!gIsFromLocal()) gRedirect("http://www.safirezendegi.com") ; 
...
//===========================================================================================
function AutoApprove($Data)
{
	//Receiving data...
	$Row      = aRecID("members" , $Data[ID]) ; 
	$NewValue = $Data[NewApproval] + 0 ; 
	
	//Precheckings...
	if(!$Row[ID])
		gBack("داده|ی مورد نظر، یافت نشد.") ;  
		
	if($Row[Approval] != 1)
		gBack("ثبت|نام شخص مورد نظر، قبلاً از حالت معلق خارج شده است.") ; 
		
	if(NewValue != 0 && $NewValue != 2)
		gBack("بروز خطا در دریافت اطلاعات") ; 
	...
	if($Row[Approval] != $NewValue)
		gBack("بروز خطا در پردازش اطلاعات") ; 
		
	//Generating the correct feedback message...
	$Cap = "ثبت|نام ".$Row[NameFirst]." ".$Row[NameLast] ;
	if($Row[Approval]==2) 
		$Cap .= " تأیید شد." ;
	else
		$Cap .= " لغو شد." ;
	...
	//Redirection...
	gBack($Cap , "admin.php?Show=Next" , "OK") ; 
}
